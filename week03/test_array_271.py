...
def test_default_construction():
    ...
    assert arr._capacity == 2, "default capacity should be 2"
    assert arr._resize_factor == 0.25, "default resize_factor should be 0.25"
    assert arr._occupancy == 0, "a brand-new array should start with occupancy 0"
    assert arr._items == [None, None], "items should be pre-sized to capacity, all None"
...
def test_custom_construction():
    ...
    assert arr._resize_factor == 0.5
    assert arr._occupancy == 0
    assert arr._items == [None, None, None, None, None]
...
def test_add_single_item_without_resizing():
    ...
    assert arr._occupancy == 1, "occupancy should go up by 1 after one add"
    assert arr._capacity == 3, "capacity should be unchanged -- there was room"
    assert arr._items[0] == "Mississippi", "the new string should land in slot 0"
    assert arr._items[1] is None, "unused slots should still be None"
    assert arr._items[2] is None, "unused slots should still be None"
...
def test_add_multiple_items_fills_slots_in_order():
    ...
    assert arr._occupancy == 3
    assert arr._items[0] == "M"
    assert arr._items[1] == "i"
    assert arr._items[2] == "s"
    assert arr._items[3] is None
...
def test_add_triggers_resize():
    ...
    assert arr._capacity > 2, "capacity should have grown past the original 2"
    assert arr._occupancy == 3, "occupancy should be 3 after three successful adds"
    assert arr._items[0] == "this"
    assert arr._items[1] == "is"
    assert arr._items[2] == "full"
